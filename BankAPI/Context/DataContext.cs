...
namespace BankAPI.Context
{
	public class DataContext : DbContext
	{
		...
		private List<CryptedAccountData> AddAccounts()
		{
			...
			var cac = _cryptographer.Encrypt(ac);
			cac.Id = 1;
			result.Add(cac);
		...
		private List<CryptedTransfer> AddTransfers()
		{
			...
			var cryptoTran = _cryptographer.EncryptTransfer(tran);
			cryptoTran.Id = 1;
			...
			cryptoTran = _cryptographer.EncryptTransfer(tran);
			cryptoTran.Id = 2;
			result.Add(cryptoTran);
		...
		private List<CryptedAccountStatus> AddStatuses()
		{
			...
			for (int i = 1; i < 4; i++)
			{
				...
				var cryptedStat = _cryptographer.EncryptStatus(stat);
				cryptedStat.Id = i;
				res.Add(cryptedStat);
			}
			return res;
